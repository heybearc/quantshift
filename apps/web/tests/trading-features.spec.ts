...
test.describe('QuantShift Trading Platform - Core Features', () => {
  ...
  test.describe('Dashboard Features', () => {
    ...
    test('Dashboard navigation links work', async ({ page }) => {
      ...
      // Verify navigation menu items are present (in LayoutWrapper)
      await expect(page.locator('nav a:has-text("Dashboard")')).toBeVisible({ timeout: 10000 });
      await expect(page.locator('nav a:has-text("Positions")')).toBeVisible();
      await expect(page.locator('nav a:has-text("Trades")')).toBeVisible();
    });
  });
  ...
  test.describe('API Integration', () => {
    test('Dashboard API endpoints respond correctly', async ({ page }) => {
      ...
      const statusPromise = page.waitForResponse(
        response => response.url().includes('/api/bot/status'),
        { timeout: 10000 }
      ).catch(() => null);
      
      await navigateTo(page, '/');
      
      // Wait for bot status API response
